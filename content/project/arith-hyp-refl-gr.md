...
-  In other words, a free finitely-generated $A$-module with an inner product of signature $(n,1)$ is said to be a hyperbolic lattice, if for each non-identity embedding $\sigma \colon \mathbb{F} \to \mathbb{R}$ the quadratic space $L \otimes_{\sigma(A)} \mathbb{R}$ is positive definite.
